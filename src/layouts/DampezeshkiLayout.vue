    ...
    <header class="fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-40 lg:hidden">
      <div class="flex items-center justify-between px-4 py-3">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-lg font-bold text-gray-900">داشبورد دامپزشکی</h1>
            <p class="text-xs text-gray-500">مدیریت کلینیک و درمانگاه</p>
          </div>
        </div>
        ...
        <div class="p-6 border-b border-gray-200">
          <div class="flex items-center gap-3 mb-2">
            <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-400 to-sky-600 flex items-center justify-center">
              <i class="ti ti-stethoscope text-white text-xl"></i>
            </div>
            <div>
              <h2 class="text-lg font-bold text-gray-900">دامپزشکی</h2>
              <p class="text-xs text-gray-500">پنل کلینیک</p>
            </div>
          </div>
        </div>
          ...
          <ul class="space-y-2">
            <li v-for="item in menuItems" :key="item.id">
              <button
                ...
                :class="currentView === item.id ? 'bg-sky-50 text-sky-600 font-medium' : 'text-gray-700 hover:text-gray-900'"
              >
                <i :class="item.icon" class="text-xl"></i>
                <span>{{ item.label }}</span>
                <span v-if="item.badge" class="mr-auto bg-sky-500 text-white text-xs px-2 py-0.5 rounded-full">
                  {{ item.badge }}
                </span>
              </button>
            </li>
            ...
            <ul class="space-y-2">
              <li v-for="item in menuItems" :key="item.id">
                <button
                  ...
                  :class="currentView === item.id ? 'bg-sky-50 text-sky-600 font-medium' : 'text-gray-700 hover:bg-gray-50'"
                >
                  <i :class="item.icon" class="text-xl"></i>
                  <span>{{ item.label }}</span>
                  <span v-if="item.badge" class="mr-auto bg-sky-500 text-white text-xs px-2 py-0.5 rounded-full">
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-[50] lg:hidden">
      <div class="flex items-center justify-around px-2 py-2">
        <button
          ...
          :class="currentView === item.id ? 'text-sky-600' : 'text-gray-600'"
        >
...
const menuItems = ref([
  ...
  { id: 'appointments', label: 'نوبت‌دهی', icon: 'ti ti-calendar', badge: '8' },
  { id: 'patients', label: 'بیماران', icon: 'ti ti-paw' },
  { id: 'medicine-orders', label: 'سفارشات دارو', icon: 'ti ti-pill', badge: '5' },
  { id: 'services', label: 'خدمات', icon: 'ti ti-briefcase' },
  { id: 'finance', label: 'امور مالی', icon: 'ti ti-wallet' }
])

const bottomNavItems = ref([
  { id: 'dashboard', label: 'خانه', icon: 'ti ti-home' },
  { id: 'appointments', label: 'نوبت‌ها', icon: 'ti ti-calendar' },
  { id: 'patients', label: 'بیماران', icon: 'ti ti-paw' },
  { id: 'finance', label: 'مالی', icon: 'ti ti-wallet' }
])
...
const getPageDescription = () => {
  const descriptions = {
    dashboard: 'آمار و عملکرد کلینیک',
    appointments: 'مدیریت نوبت‌ها و وقت‌های ویزیت',
    patients: 'لیست بیماران و سوابق پزشکی',
    'medicine-orders': 'مدیریت سفارشات دارو',
    services: 'خدمات و تعرفه‌ها',
    finance: 'تراکنش‌ها و گزارش مالی'
  }
