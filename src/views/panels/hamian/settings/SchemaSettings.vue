    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2">
          <i class="ti ti-code text-rose-600"></i>
          تنظیمات Schema Markup
        </h2>
        <p class="text-sm text-gray-500 mt-1">داده‌های ساختاریافته برای موتورهای جستجو</p>
      </div>
      <button @click="saveSchemaSettings" class="px-4 py-2 bg-gradient-to-r from-rose-600 to-pink-600 text-white rounded-lg font-bold hover:from-rose-700 hover:to-pink-700 transition-all shadow-lg">
    ...
    <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                type="text"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none text-sm"
                placeholder="حامیان"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                type="url"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none text-sm"
                placeholder="https://hamian.petoman.ir/logo.png"
              />
            </div>
      ...
      <div class="border border-gray-200 rounded-lg p-4">
        <div class="flex items-center justify-between mb-4">
          ...
          <label class="flex items-center cursor-pointer">
            <input
              v-model="schema.project.enabled"
              type="checkbox"
              class="w-5 h-5 rounded border-gray-300 text-rose-600 focus:ring-rose-500"
            />
          </label>
        ...
        <div v-if="schema.project.enabled" class="space-y-3">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-2">مدیر پروژه پیش‌فرض</label>
              <input
                v-model="schema.project.manager"
                type="text"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none text-sm"
            ...
            <div>
              <label class="block text-sm font-bold text-gray-700 mb-2">سازمان</label>
              <input
                v-model="schema.project.organization"
                type="text"
                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none text-sm"
                placeholder="حامیان"
              />
          ...
          <div class="bg-gray-900 text-green-400 rounded-lg p-4 text-xs font-mono overflow-x-auto">
            <pre>{{ JSON.stringify(generateProjectSchema(), null, 2) }}</pre>
          </div>
        </div>
      </div>
      ...
      <div class="border border-gray-200 rounded-lg p-4">
        ...
        <div v-if="schema.faq.enabled" class="space-y-3">
          <div v-for="(item, index) in schema.faq.items" :key="index" class="p-3 bg-gray-50 rounded-lg">
            <div class="grid grid-cols-1 gap-2">
              <input
                ...
                type="text"
                placeholder="سوال"
                class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 outline-none text-sm"
              />
              <textarea
                v-model="item.answer"
                rows="2"
          ...
          <button @click="addFaqItem" class="w-full py-2 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-600 hover:border-rose-500 hover:text-rose-600 transition-colors">
      ...
      <div class="border border-gray-200 rounded-lg p-4">
        ...
        <div v-if="schema.howto.enabled">
          <p class="text-xs text-gray-500 mb-3">برای راهنماهای گام به گام حمایت</p>
        </div>
      </div>
      ...
      <div class="border border-gray-200 rounded-lg p-4">
        <div class="flex items-center justify-between mb-4">
          <h3 class="font-bold text-gray-900 flex items-center gap-2">
            <i class="ti ti-calendar-event text-pink-600"></i>
            Event Schema
          </h3>
          <label class="flex items-center cursor-pointer">
            <input
              v-model="schema.event.enabled"
              type="checkbox"
              class="w-5 h-5 rounded border-gray-300 text-rose-600 focus:ring-rose-500"
            />
          </label>
        ...
        <div v-if="schema.event.enabled">
          <p class="text-xs text-gray-500">برای رویدادهای خیریه و قرارهای مهربانی</p>
        </div>
      </div>
    </div>
  </div>
...
const schema = ref({
  organization: {
    ...
    logo: 'https://hamian.petoman.ir/logo.png',
    phone: '021-12345678'
  },
  project: {
    enabled: true,
  ...
  faq: {
    enabled: false,
    items: [
      { question: 'چگونه می‌توانم حامی شوم؟', answer: 'برای حامی شدن می‌توانید...' }
    ]
  },
  howto: {
    enabled: false
...
const generateProjectSchema = () => {
  return {
    ...
    "manager": {
      "@type": "Person",
      "name": schema.value.project.manager
    },
    "organization": {
      "@type": "Organization",
      "name": schema.value.project.organization
    }
  }
}
...
const saveSchemaSettings = async () => {
  console.log('Saving Schema settings:', schema.value)
